...
import { Entry, Pair, KoconutOpener, KoconutIterable } from '../../../module';
import { EventEmitter } from 'events';

export class Flow<DataType>
  ...
  implements Iterable<Entry<number, DataType>>
{
  ...
  constructor(
    ...
    if (srcSequence != null) {
      for (const eachDatum of srcSequence) {
        this.mPentDataSize++;
        if (eachDatum instanceof Entry)
          this.mInnerDataMap.set(eachDatum.key, eachDatum.value);
        else if (eachDatum instanceof Pair)
          this.mInnerDataMap.set(eachDatum.first, eachDatum.second);
  ...
  static ofSimple<DataType>(...srcSequence: DataType[]): Flow<DataType> {
    return new Flow(
      srcSequence.map(
        (eachDatum, eachIndex) => new Entry(eachIndex, eachDatum),
      ),
    );
  }
  ...
  get dataArray(): Array<DataType> {
    return Array.from(this.sort().mInnerDataMap.entries()).map(
      (eachIterableEntry) => eachIterableEntry[1],
    );
  }
  ...
  get dataEntries(): Array<Entry<number, DataType>> {
    return Array.from(this.sort().mInnerDataMap.entries()).map(
      (eachIterableEntry) =>
        new Entry(eachIterableEntry[0], eachIterableEntry[1]),
    );
  }

  private setDatum(id: number, datum: DataType) {
    this.mInnerDataMap.set(id, datum);
    this.emit(Flow.newDatumInsertedEvent, id, datum);
  }

  private onNewDatumInserted(
    onNewDatumInsertedListener: (
      id: number,
      datum: DataType,
    ) => void | Promise<void>,
    ...
    this.once(Flow.dataScanningCompletedEvent, () =>
      this.removeListener(Flow.newDatumInsertedEvent, mediatedListener),
    );
    if (this.mInnerDataMap.size != 0)
      this.mInnerDataMap.forEach((datum, id) =>
        this.emit(Flow.newDatumInsertedEvent, id, datum),
      );
  }
}

export class KoconutFlow<DataType> extends KoconutIterable<
  Entry<number, DataType>,
  Entry<number, DataType>,
  Flow<DataType>,
  Flow<DataType>
      ...
      .setProcessor(async () => {
        ...
        if (this.data != null) {
          this.data['onNewDatumInserted'](
            async (id, datum) =>
              processedFlow['setDatum'](id, await transform(datum)),
            processedFlow,
          );
    ...
    return new Promise(async (resolve) => {
      ...
      if (this.mIsChained) resolve(this.data!);
        ...
        if (this.processor == null) resolve(this.data!['sort']());
          ...
          this.data!.once(Flow['dataScanningCompletedEvent'], () => {
